...
def test_nested_stream(img, mask):
    ...
    stream = slc.Stream(
        [slt.Flip(p=1, axis=0), slt.Flip(p=1, axis=1), slc.Stream([slt.Flip(p=1, axis=1), slt.Flip(p=1, axis=0),])]
    )
...
def test_selective_pipeline_selects_transforms_and_does_the_fusion():
    ppl = slc.SelectiveStream(
        [slt.Rotate(angle_range=(90, 90), p=1), slt.Rotate(angle_range=(-90, -90), p=1),],
        n=2,
        probs=[0.5, 0.5],
        optimize_stack=True,
    )
...
def test_nested_streams_are_not_fused_with_matrix_trf(img):
    ...
    trfs = [
        ...
        slt.Rotate(angle_range=(-90, -90), p=1),
        slc.Stream([slt.Rotate(angle_range=(90, 90), p=1),]),
        slt.Rotate(angle_range=(-90, -90), p=1),
    ]
...
def test_matrix_transforms_state_reset(img, ignore_state, pipeline):
    ...
    if pipeline:
        ppl = slc.Stream(
            [slt.Rotate(angle_range=(-180, 180), p=1, ignore_state=ignore_state), slt.Pad(pad_to=(10, 10)),]
        )
    else:
...
def test_matrix_transforms_use_cache_for_different_dc_items_raises_error(img, mask, pipeline):
    dc = slc.DataContainer((img, mask), "IM")
    if pipeline:
        ppl = slc.Stream([slt.Rotate(angle_range=(-180, 180), p=1, ignore_state=False), slt.Pad(pad_to=(10, 10)),])
    else:
...
def test_image_mask_pipeline_to_torch(img, mask):
    ppl = slc.Stream([slt.Rotate(angle_range=(90, 90), p=1), slt.Rotate(angle_range=(90, 90), p=1),],)
...
def test_image_mask_pipeline_to_torch_uint16(img, mask):
    ppl = slc.Stream([slt.Rotate(angle_range=(90, 90), p=1), slt.Rotate(angle_range=(90, 90), p=1),],)
    img, mask = ppl({"image": (img // 255).astype(np.uint16) * 65535, "mask": mask}, as_dict=False, normalize=False)
    assert img.max() == 1
    assert mask.max() == 1
    assert isinstance(img, torch.FloatTensor)
    assert isinstance(mask, torch.FloatTensor)
